...
protected:
    ParseData() {}
public:
    virtual void preform_on(ParseData& front, ParsingLog& out, ErrorList& err_out, TypeMap& types, Scope* scope) {
        err_out.push_back(SyntaxError());
    }
...
public:
    OpenComment() : text() {}
    virtual void preform_on(NewLine& front, ParsingLog& out, ErrorList& err_out, TypeMap& types, Scope* scope);
    virtual void preform_on(ParseData& front, ParsingLog& out, ErrorList& err_out, TypeMap& types, Scope* scope);
    virtual std::string to_string() { return text; }
};
...
public:
    ...
    virtual void preform_on(WrittenString& front, ParsingLog& out, ErrorList& err_out, TypeMap& types, Scope* scope);
};
...
public:
    SlashOperand();
    void preform_on(InvokeVariable& front, ParsingLog& out, ErrorList& err_out, TypeMap& types, Scope* scope);
    void preform_on(SlashOperand& front, ParsingLog& out, ErrorList& err_out, TypeMap& types, Scope* scope);
};
